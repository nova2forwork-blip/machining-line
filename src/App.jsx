...
export function partOpMatrix(logs) {
  ...
  for (const l of logs || []) {
    ...
    // key ด้วย release_id (ไม่ใช่แค่เลขที่ปล่อยงาน) — กันสอง Release ที่ไม่มีเลขปล่อยงานถูกยุบรวมกัน
    const key = `${l.release_id || releaseOrder}|${partNo}`;
    if (!byKey.has(key)) {
      byKey.set(key, { releaseOrder, partNo, partName, total: { count: 0, weight: 0, finished: 0 }, ops: {} });
    }
    const entry = byKey.get(key);
    entry.ops[op] = entry.ops[op] || { count: 0, weight: 0, finished: 0 };
    entry.ops[op].count    += pcs;
    entry.ops[op].weight   += wt;
    entry.ops[op].finished += fin;               // นับ finished แยกราย "ขั้นตอน"
    entry.total.count      += pcs;
    entry.total.weight     += wt;
  }

  // "เสร็จ (ชิ้น)" ต่อ Part = นับครั้งเดียว ไม่รวมข้ามขั้นตอน
  //   (ชิ้นเดียวผ่าน ตัด→เจาะ→บาก ไม่ใช่ "เสร็จ 3 ชิ้น") — ใช้ค่าสูงสุดของ finished รายขั้นตอน
  //   (ขั้นที่ทำเสร็จมากสุด ≈ ปลายทาง) และไม่เกินจำนวนชิ้นทั้งหมดของ Part
  for (const entry of byKey.values()) {
    const maxFin = Math.max(0, ...Object.values(entry.ops).map((o) => o.finished || 0));
    entry.total.finished = Math.min(entry.total.count, maxFin);
  }

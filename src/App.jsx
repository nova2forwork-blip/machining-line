              ...
              {releases.map((r) => {
                ...
                return (
                  <tr key={r.id} className="release-row" onClick={() => setViewPart(r)} title="กดเพื่อดูความคืบหน้าแยกขั้นตอน">
                    <td style={{ fontWeight: 600, whiteSpace: "nowrap" }}>{r.part_master?.part_no || "-"}</td>
                    <td style={{ whiteSpace: "nowrap" }}>{r.part_master?.part_name || "-"}</td>
                    <td>{r.qty}</td>
                    <td>
                      {statsLoading ? (
      ...
      <Card title="สรุปแยกตามชนิด Part (สะสมทั้งหมด)">
        <div className="table-wrap">
          <table className="data-table">
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.id}><td style={{ whiteSpace: "nowrap" }}>{r.part_no}</td><td style={{ whiteSpace: "nowrap" }}>{r.part_name}</td><td>{r.total}</td><td>{r.finished}</td><td>{fmtNum(r.weight)}</td></tr>
              ))}
            </tbody>
          </table>
        </div>
